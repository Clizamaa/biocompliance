...
import { ArrowLeft, FileText, Shield, AlertTriangle, Users, Building, Zap, Volume2, Radio, Menu, X } from 'lucide-react';
import Image from "next/image";
import Link from "next/link";
import ServiceModal from '@/components/ui/ServiceModal';

// Servicios de informes y procedimientos
const informesProcedimientos = [
  { title: 'Informe Sanitario', icon: <FileText className="h-6 w-6 text-blue-600" /> },
  { title: 'Calificación de Actividad Económica (CAE)', icon: <Building className="h-6 w-6 text-blue-600" /> },
  { title: 'Planos de riesgos', icon: <AlertTriangle className="h-6 w-6 text-blue-600" /> },
  { title: 'Planos de seguridad y evacuación', icon: <Shield className="h-6 w-6 text-blue-600" /> },
  { title: 'Trabajo Pesado', icon: <Users className="h-6 w-6 text-blue-600" /> },
  { title: 'Certificado condiciones adecuadas de prevención de incendios para centros de salud', icon: <Zap className="h-6 w-6 text-blue-600" /> },
  { title: 'Plan de Evacuación y Emergencias', icon: <AlertTriangle className="h-6 w-6 text-blue-600" /> },
  { title: 'Planes de Contingencias', icon: <Shield className="h-6 w-6 text-blue-600" /> },
  { title: 'Informes de Auditorías', icon: <FileText className="h-6 w-6 text-blue-600" /> },
  { title: 'Informe de Ruido Laboral', icon: <Volume2 className="h-6 w-6 text-blue-600" /> },
  { title: 'Informe de Ruido Ambiental', icon: <Volume2 className="h-6 w-6 text-blue-600" /> },
  { title: 'Estudio de Ruido', icon: <Volume2 className="h-6 w-6 text-blue-600" /> },
  { title: 'Modelación y Predicción Sonora', icon: <Volume2 className="h-6 w-6 text-blue-600" /> },
  { title: 'Procedimiento de Protección Radiológica Operacional', icon: <Radio className="h-6 w-6 text-blue-600" /> },
  { title: 'Procedimiento de Emergencia Radiológica', icon: <Radio className="h-6 w-6 text-blue-600" /> },
];

export default function InformesProcedimientosPage() {
        ...
        <section className="py-20 bg-gray-50">
          <div className="container mx-auto px-4 lg:px-6">
            <div className="text-center mb-16">
              <Badge className="bg-blue-100 text-blue-800 mb-4">Informes y Procedimientos</Badge>
              <h1 className="text-3xl lg:text-4xl font-bold text-charcoal-800 mb-4">Informes y Procedimientos Especializados</h1>
              <p className="text-xl text-slateGray-600 max-w-3xl mx-auto">
                Elaboración de informes técnicos y procedimientos especializados para el cumplimiento normativo y la gestión de riesgos en su organización.
              </p>
            </div>
            
            <div className="flex justify-center">
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                {informesProcedimientos.map((servicio, index) => (
                  <FadeIn key={index}>
                    <Card className="border border-slateGray-200 shadow-lg h-full text-center max-w-sm mx-auto hover:shadow-xl transition-shadow duration-300">
                      <CardHeader className="pb-4">
                        <div className="h-12 w-12 rounded-full bg-blue-100 flex items-center justify-center mb-4 mx-auto">
                          {servicio.icon}
                        </div>
                        <CardTitle className="text-lg text-charcoal-800 leading-tight">{servicio.title}</CardTitle>
                      </CardHeader>
                    </Card>
                  </FadeIn>
                ))}
