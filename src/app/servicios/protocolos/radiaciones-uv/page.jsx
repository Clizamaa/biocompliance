...
import { ArrowLeft, Sun, Construction, FileText, Menu, X } from 'lucide-react';
import Image from "next/image";
import Link from "next/link";
import MinsalModal from '@/components/ui/MinsalModal';

const uvServices = [
  { title: 'Guía Técnica Radiación Ultravioleta de origen Solar', icon: <Sun className="h-6 w-6 text-orange-600" /> },
  { title: 'Implementación Protocolo UV', icon: <Construction className="h-6 w-6 text-orange-600" /> },
];

export default function RadiacionesUvPage() {
        ...
        <section className="py-20 bg-gray-50">
          <div className="container mx-auto px-4 lg:px-6">
            <div className="text-center mb-16">
              <Badge className="bg-orange-100 text-orange-800 mb-4">Protocolo MINSAL</Badge>
              <h1 className="text-3xl lg:text-4xl font-bold text-charcoal-800 mb-4">Protocolo de Radiación Ultravioleta de Origen Solar</h1>
              <p className="text-xl text-slateGray-600 max-w-3xl mx-auto">
                Protegemos a los trabajadores de los riesgos de la exposición a la radiación UV solar, conforme a la Guía Técnica MINSAL.
              </p>
            </div>
            
            <div className="flex justify-center">
              <div className="flex flex-wrap justify-center gap-8">
                {uvServices.map((service, index) => (
                  <FadeIn key={index}>
                    <Card className="border border-slate-200 shadow-lg h-full text-center max-w-sm mx-auto">
                      <CardHeader className="pb-4 pt-6">
                        <div className="h-12 w-12 rounded-full bg-orange-100 flex items-center justify-center mb-4 mx-auto">
                          {service.icon}
                        </div>
