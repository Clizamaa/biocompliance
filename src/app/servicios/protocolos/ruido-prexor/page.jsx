...
import { ArrowLeft, Ear, ShieldCheck, Users, Menu, X } from 'lucide-react';
import Image from "next/image";
import Link from "next/link";
import MinsalModal from '@/components/ui/MinsalModal';

const prexorServices = [
  { title: 'Protocolo de Exposición Ocupacional a Ruido', icon: <Ear className="h-6 w-6 text-blue-600" /> },
  { title: 'Implementación PREXOR', icon: <ShieldCheck className="h-6 w-6 text-blue-600" /> },
];

export default function RuidoPrexorPage() {
        ...
        <section className="py-20 bg-gray-50">
          <div className="container mx-auto px-4 lg:px-6">
            <div className="text-center mb-16">
              <Badge className="bg-blue-100 text-blue-800 mb-4">Protocolo MINSAL</Badge>
              <h1 className="text-3xl lg:text-4xl font-bold text-charcoal-800 mb-4">Protocolo de Exposición Ocupacional a Ruido (PREXOR)</h1>
              <p className="text-xl text-slateGray-600 max-w-3xl mx-auto">
                Gestionamos la exposición a ruido en el lugar de trabajo para prevenir la hipoacusia neurosensorial, cumpliendo con la normativa PREXOR.
              </p>
            </div>
            
            <div className="flex justify-center">
              <div className="flex flex-wrap justify-center gap-8">
                {prexorServices.map((service, index) => (
                  <FadeIn key={index}>
