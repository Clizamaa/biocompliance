...
import { ArrowLeft, HardHat, FileText, ClipboardCheck, Menu, X } from 'lucide-react';
import Image from "next/image";
import Link from "next/link";
import MinsalModal from '@/components/ui/MinsalModal';

const siliceServices = [
  { title: 'Protocolo de Vigilancia del Ambiente de Trabajo y de la Salud de los Trabajadores con Exposición a Sílice', icon: <HardHat className="h-6 w-6 text-purple-600" /> },
  { title: 'Implementación Protocolo Sílice', icon: <ClipboardCheck className="h-6 w-6 text-purple-600" /> },
];

export default function SilicePage() {
  ...
  return (
    <div className="min-h-screen bg-white">
        ...
        <section className="py-20 bg-gray-50">
          <div className="container mx-auto px-4 lg:px-6">
            <div className="text-center mb-16">
              <Badge className="bg-purple-100 text-purple-800 mb-4">Protocolo MINSAL</Badge>
              <h1 className="text-3xl lg:text-4xl font-bold text-charcoal-800 mb-4">Protocolo de Vigilancia para Trabajadores Expuestos a Sílice (PLANESI)</h1>
              <p className="text-xl text-slateGray-600 max-w-3xl mx-auto">
                Aseguramos el cumplimiento del Plan Nacional de Erradicación de la Silicosis, protegiendo la salud de sus trabajadores expuestos a sílice.
              </p>
            </div>
            
            <div className="flex justify-center">
              <div className="flex flex-wrap justify-center gap-8">
                {siliceServices.map((service, index) => (
                  <FadeIn key={index}>
                    <Card className="border border-slate-200 shadow-lg h-full text-center max-w-sm mx-auto">
                      <CardHeader className="pb-4 pt-6">
                        <div className="h-12 w-12 rounded-full bg-purple-100 flex items-center justify-center mb-4 mx-auto">
                          {service.icon}
                        </div>
                        <CardTitle className="text-lg text-charcoal-800 h-16 flex items-center justify-center">{service.title}</CardTitle>
                      </CardHeader>
                    </Card>
                  </FadeIn>
                ))}
      ...
      <MinsalModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} />
