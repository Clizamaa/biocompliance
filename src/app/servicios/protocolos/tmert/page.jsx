...
import { ArrowLeft, ClipboardList, PersonStanding, BrainCircuit, Menu, X } from 'lucide-react';
import Image from "next/image";
import Link from "next/link";
import MinsalModal from '@/components/ui/MinsalModal';

const tmertServices = [
  { title: 'Protocolo de Vigilancia de Trastornos Musculoesqueléticos Relacionados al Trabajo', icon: <ClipboardList className="h-6 w-6 text-red-600" /> },
  { title: 'Implementación Protocolo TMERT', icon: <PersonStanding className="h-6 w-6 text-red-600" /> },
];

export default function TmertPage() {
        ...
        <section className="py-20 bg-gray-50">
          <div className="container mx-auto px-4 lg:px-6">
            <div className="text-center mb-16">
              <Badge className="bg-red-100 text-red-800 mb-4">Protocolo MINSAL</Badge>
              <h1 className="text-3xl lg:text-4xl font-bold text-charcoal-800 mb-4">Norma Técnica de Trastornos Musculoesqueléticos (TMERT)</h1>
              <p className="text-xl text-slateGray-600 max-w-3xl mx-auto">
                Identificamos y evaluamos los factores de riesgo de TMERT para prevenir lesiones en las extremidades superiores de los trabajadores.
              </p>
            </div>
            
            <div className="flex justify-center">
              <div className="flex flex-wrap justify-center gap-8">
                {tmertServices.map((service, index) => (
                  <FadeIn key={index}>
