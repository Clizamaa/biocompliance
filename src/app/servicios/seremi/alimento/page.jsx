...
import { ArrowLeft, Utensils, UtensilsCrossed, Truck, Warehouse, CookingPot, ShoppingBasket, Package, Factory, Store, Trash2, Building, Menu, X } from 'lucide-react';
import Image from "next/image";
import Link from "next/link";
import ServiceModal from '@/components/ui/ServiceModal';

// Dummy data for food services
const foodServices = [
  { title: 'Carros móviles', icon: <Truck className="h-6 w-6 text-yellow-600" /> },
  { title: 'Local de almacenamiento tipo bodega', icon: <Warehouse className="h-6 w-6 text-yellow-600" /> }, 
  { title: 'Local de distribución de alimentos', icon: <Truck className="h-6 w-6 text-yellow-600" /> }, 
  { title: 'Local de elaboración de alimentos', icon: <CookingPot className="h-6 w-6 text-yellow-600" /> }, 
  { title: 'Local de elaboración de alimentos con consumo', icon: <UtensilsCrossed className="h-6 w-6 text-yellow-600" /> },
  { title: 'Local de elaboración de alimentos con consumo al paso', icon: <ShoppingBasket className="h-6 w-6 text-yellow-600" /> },
  { title: 'Local de elaboración tipo envasadora', icon: <Package className="h-6 w-6 text-yellow-600" /> },
  { title: 'Local de elaboración tipo procesadora', icon: <Factory className="h-6 w-6 text-yellow-600" /> },
  { title: 'Local de expendio de alimentos', icon: <Store className="h-6 w-6 text-yellow-600" /> },
  { title: 'Local de expendio de alimentos con consumo', icon: <UtensilsCrossed className="h-6 w-6 text-yellow-600" /> },
  { title: 'Local de expendio de alimentos con consumo al paso', icon: <ShoppingBasket className="h-6 w-6 text-yellow-600" /> },
  { title: 'Local de expendio de alimentos y elaboración', icon: <Store className="h-6 w-6 text-yellow-600" /> },
  { title: 'Puestos, casetas y carros', icon: <Store className="h-6 w-6 text-yellow-600" /> },
  { title: 'Vehículo de transporte de alimentos', icon: <Truck className="h-6 w-6 text-yellow-600" /> },
  { title: 'Solicitud de destino alimentos no aptos para consumo humano', icon: <Trash2 className="h-6 w-6 text-yellow-600" /> },
  { title: 'Autorización de modificación a la infraestructura de instalaciones de alimento previamente autorizada', icon: <Building className="h-6 w-6 text-yellow-600" /> },
];
        ...
        <section className="py-20 bg-gray-50">
          <div className="container mx-auto px-4 lg:px-6">
              <div className="text-center mb-16">
                <Badge className="bg-yellow-100 text-yellow-800 mb-4">Servicios de Alimentos</Badge>
                <h1 className="text-3xl lg:text-4xl font-bold text-black-800 mb-4">Trámites y Autorizaciones de Alimentos</h1>
                <p className="text-xl text-slateGray-600 max-w-3xl mx-auto">
                  Asesoría experta para la obtención de autorizaciones sanitarias para establecimientos de alimentos.
                </p>
              </div>
                ...
                <FadeIn key={index}>
                  <Card className="border border-slateGray-200 shadow-lg h-full text-center max-w-sm mx-auto">
                    <CardHeader className="pb-2">
                      <div className="h-12 w-12 rounded-full bg-yellow-100 flex items-center justify-center mb-4 mx-auto">
                        {service.icon}
                      </div>
