...
import { ArrowLeft, Factory, Menu, X } from 'lucide-react';
import Image from "next/image";
import Link from "next/link";
import ServiceModal from '@/components/ui/ServiceModal';

// Dummy data for industrial qualification services
const industrialQualificationServices = [
  { title: 'CAE o Calificación de industrias, actividades de impacto similar al industrial e infraestructura', icon: <Factory className="h-6 w-6 text-gray-600" /> },
];

export default function CalificacionIndustrialPage() {
        ...
        <section className="py-20 bg-gray-50">
          <div className="container mx-auto px-4 lg:px-6">
              <div className="text-center mb-16">
                <Badge className="bg-gray-100 text-gray-800 mb-4">Calificación Industrial</Badge>
                <h1 className="text-3xl lg:text-4xl font-bold text-black-800 mb-4">Gestión de Calificación Industrial</h1>
                <p className="text-xl text-slateGray-600 max-w-3xl mx-auto">
                  Asesoría para la obtención del informe de calificación industrial, un requisito para la obtención de la patente municipal.
                </p>
              </div>
            
            <div className="flex justify-center">
              <div className="w-full flex justify-center">
              {industrialQualificationServices.map((service, index) => (
                <FadeIn key={index}>
                  <Card className="border border-slateGray-200 shadow-lg h-full text-center max-w-sm mx-auto">
                    <CardHeader className="pb-2">
                      <div className="h-12 w-12 rounded-full bg-gray-100 flex items-center justify-center mb-4 mx-auto">
                        {service.icon}
                      </div>
