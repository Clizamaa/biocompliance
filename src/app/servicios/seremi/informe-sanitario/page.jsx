...
import { ArrowLeft, FileText, School, Building, Dog, Users, Menu, X } from 'lucide-react';
import Image from "next/image";
import Link from "next/link";
import ServiceModal from '@/components/ui/ServiceModal';

// Dummy data for sanitary report services
const reportServices = [
  { title: 'Informe sanitario para establecimientos educacionales', icon: <School className="h-6 w-6 text-indigo-600" /> },
  { title: 'Informe sanitario para locales de uso público', icon: <Building className="h-6 w-6 text-indigo-600" /> },
  { title: 'Informe sanitario para establecimientos industriales y/o comerciales', icon: <Building className="h-6 w-6 text-indigo-600" /> },
  { title: 'Informe sanitario para la modificación de condiciones iniciales de funcionamiento de establecimientos educacionales', icon: <School className="h-6 w-6 text-indigo-600" /> },
  { title: 'Informe sanitario para centros de mantención temporal de mascotas o animales de compañía', icon: <Dog className="h-6 w-6 text-indigo-600" /> },
  { title: 'Eventos Masivos: Informe técnico sobre condiciones sanitarias', icon: <Users className="h-6 w-6 text-indigo-600" /> },
];

export default function InformeSanitarioPage() {
        ...
        <section className="py-20 bg-gray-50">
          <div className="container mx-auto px-4 lg:px-6">
              <div className="text-center mb-16">
                <Badge className="bg-indigo-100 text-indigo-800 mb-4">Informe Sanitario</Badge>
                <h1 className="text-3xl lg:text-4xl font-bold text-black-800 mb-4">Gestión de Informes Sanitarios</h1>
                <p className="text-xl text-slateGray-600 max-w-3xl mx-auto">
                  Elaboración y gestión de informes sanitarios requeridos para diversos tipos de establecimientos y actividades.
                </p>
              </div>
