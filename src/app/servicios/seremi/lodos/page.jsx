...
import { ArrowLeft, Recycle, Droplets, Menu, X } from 'lucide-react';
import Image from "next/image";
import Link from "next/link";
import ServiceModal from '@/components/ui/ServiceModal';

// Dummy data for sludge services
const sludgeServices = [
  { title: 'Aprobación de proyecto de sistema de manejo de lodos de plantas de tratamiento de aguas servidas.', icon: <Recycle className="h-6 w-6 text-amber-700" /> },
  { title: 'Autorización de funcionamiento de sistema de manejo de lodos de plantas de tratamiento de aguas servidas.', icon: <Droplets className="h-6 w-6 text-amber-700" /> }, 
];

export default function LodosPage() {
        ...
        <section className="py-20 bg-gray-50">
          <div className="container mx-auto px-4 lg:px-6">
              <div className="text-center mb-16">
                <Badge className="bg-amber-100 text-amber-800 mb-4">Lodos</Badge>
                <h1 className="text-3xl lg:text-4xl font-bold text-black-800 mb-4">Gestión y Aplicación de Lodos</h1>
                <p className="text-xl text-slateGray-600 max-w-3xl mx-auto">
                  Asesoría en la gestión, almacenamiento y aplicación de lodos provenientes de plantas de tratamiento de aguas servidas.
                </p>
              </div>
            
            <div className="flex justify-center">
              <div className="grid md:grid-cols-2 gap-8">
              {sludgeServices.map((service, index) => (
                <FadeIn key={index}>
                  <Card className="border border-slateGray-200 shadow-lg h-full text-center max-w-sm mx-auto">
                    <CardHeader className="pb-2">
                      <div className="h-12 w-12 rounded-full bg-amber-100 flex items-center justify-center mb-4 mx-auto">
                        {service.icon}
                      </div>
