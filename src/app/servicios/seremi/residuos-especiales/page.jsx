...
import { ArrowLeft, Truck, ClipboardCheck, Biohazard, Archive, Menu, X } from 'lucide-react';
import Image from "next/image";
import Link from "next/link";
import ServiceModal from '@/components/ui/ServiceModal';

// Dummy data for special waste services
const specialWasteServices = [
  { title: 'Autorización de transporte de residuos peligrosos fuera de la región.', icon: <Truck className="h-6 w-6 text-slate-600" /> },
  { title: 'Aprobación de Plan de Manejo de Residuos de Establecimientos de Atención de Salud (REAS).', icon: <ClipboardCheck className="h-6 w-6 text-slate-600" /> },
  { title: 'Autorización de funcionamiento de sitio de almacenamiento de Residuos de Establecimientos de Atención de Salud (REAS).', icon: <Biohazard className="h-6 w-6 text-slate-600" /> },
  { title: 'Autorización de sistema de tratamiento de Residuos de Establecimientos de Atención de Salud (REAS).', icon: <Biohazard className="h-6 w-6 text-slate-600" /> },
  { title: 'Autorización sanitaria para la instalación de una bodega de almacenamiento de residuos peligrosos.', icon: <Archive className="h-6 w-6 text-slate-600" /> },
];

export default function ResiduosEspecialesPage() {
        ...
        <section className="py-20 bg-gray-50">
          <div className="container mx-auto px-4 lg:px-6">
              <div className="text-center mb-16">
                <Badge className="bg-slate-100 text-slate-800 mb-4">Residuos Especiales</Badge>
                <h1 className="text-3xl lg:text-4xl font-bold text-black-800 mb-4">Manejo de Residuos Especiales</h1>
                <p className="text-xl text-slateGray-600 max-w-3xl mx-auto">
                  Asesoría en la gestión y declaración de residuos especiales, asegurando el cumplimiento de la normativa vigente.
                </p>
              </div>
            
            <div className="flex justify-center">
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
              {specialWasteServices.map((service, index) => (
                <FadeIn key={index}>
                  <Card className="border border-slateGray-200 shadow-lg h-full text-center max-w-sm mx-auto">
                    <CardHeader className="pb-2">
                      <div className="h-12 w-12 rounded-full bg-slate-100 flex items-center justify-center mb-4 mx-auto">
                        {service.icon}
                      </div>
