...
import { ArrowLeft, FileCheck, Archive, Truck, Building, Recycle, Menu, X } from 'lucide-react';
import Image from "next/image";
import Link from "next/link";
import ServiceModal from '@/components/ui/ServiceModal';

// Dummy data for non-hazardous waste services
const nonHazardousWasteServices = [
  { title: 'Aprobación de proyecto de instalación de tratamiento o disposición final de residuos industriales no peligrosos dentro del predio', icon: <FileCheck className="h-6 w-6 text-lime-600" /> },
  { title: 'Autorización del sitio de almacenamiento de residuos no peligrosos', icon: <Archive className="h-6 w-6 text-lime-600" /> },
  { title: 'Autorización para transporte de residuos industriales no peligrosos', icon: <Truck className="h-6 w-6 text-lime-600" /> },
  { title: 'Autorización para realizar tratamiento o disposición final de residuos industriales no peligrosos fuera del predio', icon: <Building className="h-6 w-6 text-lime-600" /> },
  { title: 'Aprobación de proyecto de la instalación de valorización o eliminación de residuos no peligrosos', icon: <Recycle className="h-6 w-6 text-lime-600" /> },
  { title: 'Autorización de la instalación de valorización o eliminación de residuos no peligrosos', icon: <Building className="h-6 w-6 text-lime-600" /> },
  { title: 'Autorización de instalación de tratamiento o disposición final de residuos industriales no peligrosos dentro del predio', icon: <Building className="h-6 w-6 text-lime-600" /> },
];

export default function ResiduosNoPeligrososPage() {
        ...
        <section className="py-20 bg-gray-50">
          <div className="container mx-auto px-4 lg:px-6">
              <div className="text-center mb-16">
                <Badge className="bg-lime-100 text-lime-800 mb-4">Residuos No Peligrosos</Badge>
                <h1 className="text-3xl lg:text-4xl font-bold text-black-800 mb-4">Gestión de Residuos No Peligrosos</h1>
                <p className="text-xl text-slateGray-600 max-w-3xl mx-auto">
                  Soporte en la gestión y autorización de instalaciones para el manejo de residuos sólidos no peligrosos.
                </p>
              </div>
            
            <div className="flex justify-center">
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
              {nonHazardousWasteServices.map((service, index) => (
                <FadeIn key={index}>
                  <Card className="border border-slateGray-200 shadow-lg h-full text-center max-w-sm mx-auto">
                    <CardHeader className="pb-2">
                      <div className="h-12 w-12 rounded-full bg-lime-100 flex items-center justify-center mb-4 mx-auto">
                        {service.icon}
                      </div>
